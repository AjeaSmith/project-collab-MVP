...
export async function getUserById(userId) {
	try {
		const user = await database.listDocuments(databaseId, usersCollectionId, [
...
export async function editProfile(data) {
	...
	try {
		...
		const profileToUpdate = await database.listDocuments(
			databaseId,
			usersCollectionId,
			[Query.equal("accountId", currentUser.$id)]
		);

		const profile = profileToUpdate.documents[0];
